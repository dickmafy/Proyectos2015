<?php
global $meta, $portfolio, $post, $show_slideshow, $_query_has_videos, $isFullsizeGallery, $main_homepage, $isFullsizeVideo, $max_retina_sizes;
			...
			<div class="rel">
				...
				<div id="showtitle" class="clearfix">
					<div class="clearfix title">
					  <?php if ( $main_homepage && get_option_max('fullsize_show_title') == 'true' || $meta['max_show_page_fullsize_title'] == 'true' ) : ?>
						<span class="clearfix imagetitle"><a href="#">description</a></span>
						<?php endif; ?>
						<?php if ( get_option_max('fullsize_show_title_excerpt') == 'true' ){ ?><span class="imagecaption" style="display: none;">caption</span><?php } ?>
						<span class="imagecount small">1/1</span>
 					  <?php if ( get_option_max('fullsize_show_title_excerpt') == 'true' ) : ?>
						<?php if ( get_option_max('fullsize_show_title_more') == 'true' ){ ?><span class="more"><a href="#">Show More</a></span><?php } ?>
						<?php endif; ?>
					</div>
				</div>
				<?php } ?>
				...
				<div id="thumbnailContainer" <?php if ( !$show_slideshow ) echo ('style="display: none"') ?> <?php if(get_option_max('fullsize_thumb_center') == 'true') echo 'class="centered"' ?>>
                ...
                $_video_lightbox      = !empty($_post_meta[MAX_SHORTNAME . '_photo_lightbox_video_show']) && $show_title_link == "false" ? $_post_meta[MAX_SHORTNAME.'_photo_lightbox_video_show'] : "" ;
                ...
                if( get_option_max('fullsize_use_square') == 'true' ){
                  ...
                  $imgUrl = max_get_image_path(get_the_ID(), 'mobile');
								...
								<a <?php echo $data_add ?> class="item <?php echo $_item_type .' '; echo $img_greyscale ?>" href="<?php echo $imgUrl_big; ?>" title="<?php the_title() ?>" data-1x="<?php echo $imgUrl_big ?>" data-2x="<?php echo $imgUrl_big ?>">
									...
									<img src="<?php echo $imgUrl; ?>" height="<?php get_option_max('fullsize_thumb_height',true) ?>" <?php echo $_img_width ?> class="is-thumb img-color" title="<?php strip_tags(the_title()) ?>" alt="<?php if(get_the_excerpt()){ echo strip_tags(get_the_excerpt()); }else{ echo strip_tags(get_the_title()); } ?>" data-1x="<?php echo $imgUrl ?>" />
				...
				?>
					...
					function superbgimage_show(img) {
						...
						jQuery('#fullsize a' + "[rel='" + jQuery.superbg_imgActual + "']").livequery(function(){
							...
							jQuery('#showtitle span.imagetitle a').text( jQuery('#thumbnails a' + "[rel='" + img + "'] img").attr('title') );
