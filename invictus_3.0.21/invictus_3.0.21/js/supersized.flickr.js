...
(function($){
	...
	jQuery(document).ready(function() {
		jQuery('body').prepend('<div id="supersized-loader"></div>').prepend('<div id="supersized"></div>');
	});
	...
	$.supersized = function( options ) {
		...
		//Default elements
		var element = jQuery('#supersized');		//Supersized container
	...
	switch(options.sort_direction){
			...
			type: 'GET',
  			...
  			success: function(data){
				...
				if (options.slides.length > 1){
					...
					jQuery("<img/>").attr("src", options.slides[loadPrev].image).appendTo(element).wrap('<a ' + imageLink + linkTarget + '></a>');
				}

				//Set current image
				imageLink = (options.slides[currentSlide].url) ? "href='" + options.slides[currentSlide].url + "'" : "";
				jQuery("<img/>").attr("src", options.slides[currentSlide].image).appendTo(element).wrap('<a class="activeslide" ' + imageLink + linkTarget + '></a>');
				...
				if (options.slides.length > 1){
					...
					jQuery("<img/>").attr("src", options.slides[loadNext].image).appendTo(element).wrap('<a ' + imageLink + linkTarget + '></a>');
				}
		...
		jQuery('#controls-wrapper').hide();	//Hide controls to be displayed
		...
		var loadFunc = function(){
			...
			jQuery('#controls-wrapper').show();		//Display controls
			...
			if (options.thumbnail_navigation == 1){
				...
				jQuery('#prevthumb').show().html(jQuery("<img/>").attr("src", options.slides[prevThumb].thumb));

				//Load next thumbnail
				currentSlide == options.slides.length - 1 ? nextThumb = 0 : nextThumb = currentSlide + 1;
				jQuery('#nextthumb').show().html(jQuery("<img/>").attr("src", options.slides[nextThumb].thumb));

			}

			resizenow();	//Resize background image

			if (options.slide_captions) jQuery('#slidecaption').html(options.slides[currentSlide].title);		//Pull caption from array
			...
			if (options.slideshow){
				...
				if (options.slide_counter){	//Initiate slide counter if active
					...
					jQuery('#slidecounter .slidenumber').html(currentSlide + 1);		//Pull initial slide number from options
				...
				if (options.navigation){
				    ...
				    jQuery(pauseplay).click(function() {
						...
						if (isPaused){
							...
							if (jQuery(pauseplay).hasClass('fullsize-start')) jQuery(pauseplay).removeClass('fullsize-start').addClass('fullsize-stop');
			        		...
			        		if (jQuery(pauseplay).hasClass('fullsize-stop')) jQuery(pauseplay).removeClass('fullsize-stop').addClass('fullsize-start');
		...
		if (options.keyboard_nav){
			...
			jQuery(document.documentElement).keydown(function (event) {
				...
				if ((event.keyCode == 37) || (event.keyCode == 40)) { //Left Arrow or Down Arrow
					...
					if (jQuery('#prevslide').attr('src')) jQuery('#prevslide').attr("src", image_path + "back.png");		//If image, change back button to active

				} else if ((event.keyCode == 39) || (event.keyCode == 38)) { //Right Arrow or Up Arrow

					if (jQuery('#nextslide').attr('src')) jQuery('#nextslide').attr("src", image_path + "forward.png");	//If image, change next button to active
			...
			jQuery(document.documentElement).keyup(function (event) {
				...
				if ((event.keyCode == 37) || (event.keyCode == 40)) { //Left Arrow or Down Arrow
					...
					if (jQuery('#prevslide').attr('src')) jQuery('#prevslide').attr("src", image_path + "back_dull.png");	//If image, change back button to normal
					...
					if (jQuery('#nextslide').attr('src')) jQuery('#nextslide').attr("src", image_path + "forward_dull.png");	//If image, change next button to normal
		...
		function resizenow() {
			return element.each(function() {
		  		...
		  		var t = jQuery('img', element);

		  		//Resize each image seperately
		  		jQuery(t).each(function(){

					var ratio = (jQuery(this).height()/jQuery(this).width()).toFixed(2);	//Define image ratio
				...
				if (options.image_protect){
					...
					jQuery('img', element).bind("contextmenu",function(){
						return false;
					});
					jQuery('img', element).bind("mousedown",function(){
						return false;
					});
		...
		function nextslide() {
			...
			jQuery("<img/>").attr("src", options.slides[loadSlide].image).appendTo(element).wrap("<a " + imageLink + linkTarget + "></a>");	//Append new image
			...
			if (options.thumbnail_navigation == 1){
				...
				jQuery('#prevthumb').html(jQuery("<img/>").attr("src", options.slides[prevThumb].thumb));

				//Load next thumbnail
				nextThumb = loadSlide;
				jQuery('#nextthumb').html(jQuery("<img/>").attr("src", options.slides[nextThumb].thumb));
			...
			if (options.slide_counter){
			    jQuery('#slidecounter .slidenumber').html(currentSlide + 1);
			}

			//Update captions
		    if (options.slide_captions){
		    	(options.slides[currentSlide].title) ? jQuery('#slidecaption').html(options.slides[currentSlide].title) : jQuery('#slidecaption').html('');
		    }
	    	...
	    	switch(options.transition){
	    		...
	    		case 1:    //Fade
	    		    ...
	    		    break;
	    		case 2:    //Slide Top
	    		    nextslide.animate({top : -jQuery(window).height()}, 0 ).show().animate({ top:0 }, options.transition_speed, function(){ afterAnimation(); });
	    		    break;
	    		case 3:    //Slide Right
	    			nextslide.animate({left : jQuery(window).width()}, 0 ).show().animate({ left:0 }, options.transition_speed, function(){ afterAnimation(); });
	    			break;
	    		case 4:    //Slide Bottom
	    			nextslide.animate({top : jQuery(window).height()}, 0 ).show().animate({ top:0 }, options.transition_speed, function(){ afterAnimation(); });
	    			break;
	    		...
	    		case 6:    //Carousel Right
	    			nextslide.animate({left : jQuery(window).width()}, 0 ).show().animate({ left:0 }, options.transition_speed, function(){ afterAnimation(); });
					currentslide.animate({ left: -jQuery(window).width() }, options.transition_speed );
	    			break;
		...
		function prevslide() {
		    ...
		    if ( currentslide.length == 0 ) currentslide = jQuery(element).find('a:first');	//If end of set, note this is first slide
		    var nextslide =  currentslide.prev().length ? currentslide.prev() : jQuery(element).find('a:last');
			var prevslide =  nextslide.next().length ? nextslide.next() : jQuery(element).find('a:first');
			...
			imageLink = (options.slides[loadSlide].url) ? "href='" + options.slides[loadSlide].url + "'" : "";	//If link exists, build it
			jQuery("<img/>").attr("src", options.slides[loadSlide].image).prependTo(element).wrap("<a " + imageLink + linkTarget + "></a>");	//Append new image
