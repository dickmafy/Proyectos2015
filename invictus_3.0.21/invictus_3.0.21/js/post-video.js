/** Javascript to load the videos on demand **/
	...
	function onVideoPauseTrigger(){
		...
		// is selfhosted video or touch device
	...
	function prepareVideoComponents(triggerClass, container){
		...
		if ( dataObject.fullsize_autoplay_video == 'false' && isTouch !== true && json.post_type !== 'youtube_embed' ) {
			$_fsg_button.fadeIn(fadeInterval);
		}
	...
	if($_fullvid) jQuery('#fullsizeVideoHolder, #fullsizeVideo').css({width:100+"%",height:100+"%"}) // its a single full size video template
		...
		if (json.post_type === 'youtube_embed') {
			...
			if(!ytplayer){
				jQuery('#youtubeplayer').append('<iframe id="youtubePlayer_Frame" type="text/html" width="100%" height="100%" src="http://www.youtube.com/embed/'+json.embedded_code+'?enablejsapi=1&version=3&modestbranding=1&autohide=1&hd=1&showinfo=0&rel=0&wmode=transparent" frameborder="0"></iframe>');
			}else{
