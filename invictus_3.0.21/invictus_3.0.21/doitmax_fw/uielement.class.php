					...
					switch($box['type']) {
						...
						case 'sortable_gallery_checkbox':
							...
							$output .= '<script type="text/javascript">
											jQuery( "#sortable'.$box['id'].'" ).sortable({ forcePlaceholderSize: true, placeholder: "ui-state-highlight" });
										</script>';
						...
						case 'upload_image':
							...
							foreach( $data as $index => $value ){
								$_img = "";
								$_img_tmp = wp_get_attachment_image_src( $value['imgID'], "mobile" );
								$_img = $_img_tmp[0];
								if(!$_img){
									$_img = MAX_FW_DIR.'admin/images/no_image.jpg';
								}

								$output .= '<div class="clearfix max_meta_box max_image_row">
												<div class="media_upload_image">
													<div class="max_pic_preview"><img src="'. $_img .'" /></div>
								...
								foreach( $cropping_array as $val => $option ){
									...
									if(empty($value['cropping'])){
										$val = 'c';
										$value['cropping'] = 'c';
									}

									if(!empty($value['cropping']) && $value['cropping'] == $val){
						...
						case 'single_image':
							...
							$_img = "";
							$_img_tmp = wp_get_attachment_image_src( $data, "mobile" );
							$_img = $_img_tmp[0];
							if(!$_img){
								$_img = MAX_FW_DIR.'admin/images/no_image.jpg';
							}
							...
							$output .= '<script type="text/javascript">
								jQuery(document).ready(function($) {
									...
									function colorPickerTrigger(val){
										var add = "";
										if(val.charAt(0) != "#") add = "#";
										jQuery("#cp_'.$box['id'] .' div").css({"backgroundColor": add+val });
										jQuery("#colorpicker_'. $box['id'] .'").val(add+val);
									}

									jQuery("#colorpicker_'. $box['id'] .'").ColorPicker({
									...
									jQuery("#cp_'. $box['id'] .'").next("input").change(function(){
									   jQuery("#cp_'. $box['id'] .' div").css({"backgroundColor": jQuery(this).val(), "backgroundImage": "none"});
									})

								});
							</script>';

							$output .='<div id="cp_'. $box['id'] .'" class="cp_box">
								<div class="color_display" style="background-color:'.$data.'; background-image:  none; }"></div>
							</div>
							<div class="max_style_holder max_style_'. $box['type'] .'">
								<input  maxlength="7" type="text" name="'. $box['id'] .'" id="colorpicker_'. $box['id'] .'" value="'.$data.'" class="cp_input max_control_input" />
							</div>';
