...
/* =============================================================================
   Function to get the image ID from a given image URL
   ========================================================================== */

  function max_get_attachment_id_from_url( $attachment_url = '' ) {
...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
  ...
  function mr_image_resize( $url, $width = null, $height = null, $crop = true, $align = '', $retina = false ) {
    ...
    if ( !file_exists( $dest_file_name ) ) {
      ...
      if ( IMAGETYPE_PNG == $orig_type || "image/png" == $orig_type && function_exists('imageistruecolor') && !imageistruecolor( $image ) ) {
...
function mr_common_info($args) { global $der;
  ...
  // get native wordpress meta data for this image url
  $wp_image_id  = max_get_attachment_id_from_url( $url ); // get the post attachment id
  $post         = get_post($wp_image_id);
  $meta         = wp_get_attachment_metadata( $post->ID );

	if ( is_array( $meta ) && array_key_exists( 'width', $meta ) && array_key_exists( 'height', $meta ) ) :

    // get the original image dimensions for the "full" image, otherwise fallback to default
    $orig_width   = !empty($meta[ 'sizes' ][ 'full' ][ 'width' ]) ? $meta[ 'sizes' ][ 'full' ][ 'width' ] : $meta[ 'width' ]; // the image width
    $orig_height  = !empty($meta[ 'sizes' ][ 'full' ][ 'height' ]) ? $meta[ 'sizes' ][ 'full' ][ 'height' ] : $meta[ 'height' ]; // the image height
    $orig_type    = $post->post_mime_type; // get the post attachment mime type
    ...
    if( !ini_get('allow_url_fopen') && function_exists('curl_init') ) {
      ...
      $size = @getimagesize('file'); //Get image size
