...
if ( ! function_exists( 'max_query_posts' ) ):
	...
	function max_query_posts($showposts, $catArray, $random = false, $offset = false, $order = 'DESC') {
		...
		global $post, $max_random_posts_query, $paged;
    ...
    $seed = date('Ymdhi');  // Use date('Ymdh') to get an hourly change
    ...
    $max_random_posts_query = ''; // Turn off filter
...
function max_get_timthumb_image( $return = false, $p_id = false, $img_size = 'full' ){
  ...
  $use_timthumb      = get_option_max('image_use_timthumb');
	...
	if ( ( $detect->isMobile() && !$detect->isTablet() ) && !empty($imgDimensions1x) && is_array($imgDimensions1x)) { // its a mobile phone device so we need other images to display properly
    ...
    if( empty($tpl_show_lightbox) || $tpl_show_lightbox == 'false' ) :	// check if option to show lightbox on this page template is enabled
  		// Get the image link
  		$output .=	'<a href="'. $imgFull[0] .'" data-rel="prettyPhoto"' . $title . ' data-link="'. get_permalink($post_id).'">';
		endif;
  ...
  if($resize_images === true) :
    ...
    $data1x_img = max_get_custom_image_url(get_post_thumbnail_id($post_id), false, $_dimensions['width'], $_dimensions['height'], get_cropping_direction( get_post_meta($post_id, $shortname.'_photo_cropping_direction_value', true) ), false, true );

  else : // lets get the cropped WP version
