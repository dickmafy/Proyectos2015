...
function max_widgets_init() {
	...
	$_sidebar_array = array(
		...
		array( "id" => "gallery-galleria", "name" => __( 'Galleria Template Sidebar', MAX_SHORTNAME), "description" =>  __( 'Galleria Template Widget Area', MAX_SHORTNAME) ),
		...
		array( "id" => "gallery-lightbox", "name" => __( 'Sidebar Lightbox Gallery', MAX_SHORTNAME), "description" =>  __( 'Sidebar for lightbox gallery templates', MAX_SHORTNAME) ),
	);
...
if ( ! function_exists( 'max_get_custom_sidebar' ) ):

  function max_get_custom_sidebar($default_sidebar) {

    global $post;

	  // check if we have a custom sidebar
	  $meta_custom_sidebar = get_post_meta($post->ID, MAX_SHORTNAME.'_sidebar_select', true);

    // set the default sidebar for this template
    $sidebar_string = $default_sidebar;

    // check for the custom sidebar
	  if( !empty($meta_custom_sidebar) && $meta_custom_sidebar != "false") :
	    $sidebar_string = 'sidebar-'.$meta_custom_sidebar;
	  endif;
