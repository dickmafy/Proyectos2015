...
class UIElement_Photo extends UIElement {
	...
	public function getFullsizeGalleryMetaBox() {

		global $cropping_array, $post_id;

		$this->createMetabox(array(
			'id' =>  MAX_SHORTNAME . "_image_fsg_settings",
			'title' => __('Fullsize Gallery Settings for Photo Post', MAX_SHORTNAME),
			'priority' => "high"
		));
		...
		// Background Color
		$this->addColorPicker(array(
			'id' => MAX_SHORTNAME."_photo_item_background_color",
			'label' => __('Background Color for Fullsize Gallery', MAX_SHORTNAME),
			'standard' => '',
			'desc' => __('Choose the background color to display at back of this photo in a fullsize gallery', MAX_SHORTNAME)
		));

		// Custom Link Target Group
		$this->addGroupOpen(array(
			"id" => MAX_SHORTNAME.'_photo_group_custom_link',
			"dependency" => MAX_SHORTNAME.'_photo_item_type_value::{not}external',
			"display" => false
		));

			// Show the fsg link on the title
			$this->addDropdown(array(
				"id" => MAX_SHORTNAME."_photo_item_fsg_link",
				"label" => __('Link on fullsize title', MAX_SHORTNAME),
				"options" => array( 'true'=>'Yes', 'false' => 'No' ),
				"standard" => "true",
				"desc" => __('Do you want to show a link on the photo title in a fullsize gallery?', MAX_SHORTNAME )
			));

			// Link or no link?
			$this->addGroupOpen(array(
				"id" => MAX_SHORTNAME.'_photo_group_link',
				"dependency" => MAX_SHORTNAME.'_photo_item_fsg_link::true',
				"display" => false
			));

  			// Enable Lightbox on Videos in a full size gallery
  			$this->addDropdown(array(
  				"id"            => MAX_SHORTNAME."_photo_lightbox_video_show",
  				"label"         => __('Show Lightbox Videos in fullsize gallery', MAX_SHORTNAME),
  				"options"       => array( 'false'=>'No', 'true' => 'Yes' ),
  				"standard"      => "false",
  				"dependency"    => MAX_SHORTNAME.'_photo_item_type_value::lightbox',
  				"desc"          => __('Do you want to show lightbox type videos in a lightbox in the fullsize gallery?', MAX_SHORTNAME )
  			));

				// Custom Link url
				$this->addInput(array(
					"id"       => MAX_SHORTNAME."_photo_item_custom_link",
					"label"    => __('Custom link for Fullsize Gallery.', MAX_SHORTNAME),
					"size"     => "480",
					"desc"     => __('You can enter a custom URL to link to when you click on the post title in the full size gallery view of this photo. Leave blank to link to the default photo post link. Remember to add a http:// in front of your URL.', MAX_SHORTNAME)
				));

				// Custom link target
				$this->addDropdown(array(
					"id" => MAX_SHORTNAME."_photo_item_custom_link_target",
					"label" => __('Link target', MAX_SHORTNAME),
					"options" => array('_blank'=>'_blank', '_new' => '_new', '_self' => '_self' ),
					"standard" => "_blank",
					"desc" => __('Choose the link target to open your custom link.', MAX_SHORTNAME )
				));
		...
		$this->addGroupClose(array(
			"id" => MAX_SHORTNAME.'_photo_group_custom_link_close'
		));

  }

	public function getPhotoMetaBox() {

		global $cropping_array, $post_id;

		$this->createMetabox(array(
			'id' =>  MAX_SHORTNAME . "_image_detail_settings",
			'title' => __('Photo Settings', MAX_SHORTNAME),
			'priority' => "high"
		));
		...
		$this->addDropdown(array(
			...
			'options' => array('lightbox'=>'Lightbox',
							   ...
							   'disable_link'    => __('Disable Link', MAX_SHORTNAME)
							  ),
			'standard' => "lightbox",
		));
		...
		$this->addGroupOpen(array(
			...
			$this->addDropdown(array(
				...
				"options" => array('photo'=>'Featured Image', 'youtube'=>'YouTube-Video', 'vimeo'=>'Vimeo-Video', 'custom'=>'Custom Link'),
				"standard" => "photo",
				"display" => true,
			...
			// Custom Lightbox url
			$this->addInput(array(
				"id" => MAX_SHORTNAME."_photo_item_custom_lightbox",
				"label" => __('Custom URL for Lightbox link.', MAX_SHORTNAME),
				"size" => "640",
				'dependency' => MAX_SHORTNAME.'_photo_lightbox_type_value::custom',
				"display" => false,
				"desc" => __('You can enter a custom URL to link to open in the lightbox, for example a link to a PDF. Leave blank to show the photo post featured image in the lightbox. Remeber to add a http:// in front of your URL.', MAX_SHORTNAME)
			));
		...
		$this->addDropdown(array(
			...
			"standard" => "c",
			"desc" => __('Choose the position from where your thumbnail of the photo will be cropped if cropping is activated in your Invictus Theme Settings.', MAX_SHORTNAME )
		));
