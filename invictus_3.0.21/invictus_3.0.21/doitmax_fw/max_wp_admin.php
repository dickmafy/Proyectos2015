...
function max_admin_add_init($hook) {
	...
	if ( $hook == 'post.php' ||
		...
		wp_register_script('jquery-ui',"http://ajax.googleapis.com/ajax/libs/jqueryui/1.9.2/jquery-ui.min.js", array('jquery'), '1.9.2');
...
/*-----------------------------------------------------------------------------------*/
  ...
  function max_get_option_sidebar($value){
    ...
    ?>
		...
		<br />&nbsp;
		<p>
      <input type="button" class="button action" name="max_button_<?php echo $value['id'] ?>" value="<?php _e('Create Sidebar', MAX_SHORTNAME) ?>" />
		</p>

    <ul id="sb_list_<?php echo $value['id'] ?>" class="ui-sortable">
	  <?php
	  if( is_array($sidebars) ) : // we have some sidebars
        ...
        echo "<li id='sidebar_".$index."' class='ui-state-default'><span></span>".$sidebar[0]."<a href='#' class='remove'>Delete</a><input type='hidden' name='".$value['id']."[".$index."]' value='".$sidebar[0].",".$sidebar[1]."' /></li>";
	    endforeach;
    ...
    <script type="text/javascript">
        ...
        $ul = jQuery('#option_<?php echo $value['id']; ?>'), // store the list

        // add the click event to the sidebar button
        jQuery('#option_<?php echo $value['id']; ?>').on('click', 'input[name="max_button_<?php echo $value['id'] ?>"]', function(){
          ...
          if($val != ""){
            ...
            $li = jQuery("<li id='sidebar_"+num+"' class='ui-state-default'><span></span>" + $val + "<a href='#' class='remove'>Delete</a>" + "<input type='hidden' name='<?php echo $value['id'] ?>["+num+"]' value='" + $val + "," +$val.toLowerCase().split(' ').join('-') +"' /></li>");

            jQuery('#sb_list_<?php echo $value['id'] ?>').append($li);
  ...
  function max_get_option_subhead($value){
  	...
  	<div class="max-sub-header">
  		<h2>
  			<img src="<?php echo MAX_FW_DIR ?>admin/images/icons/cog.png" />
  			<?php echo $value['name']; ?>
  		</h2>
  	</div>
  	<?php
  ...
  function max_get_option_text($value){
  	?>
  	<div class="max_style_holder max_style_<?php echo $value['type'] ?>">
  		<input class="max_control_input" name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" value="<?php if ( get_option( $value['id'] ) != "") { echo stripslashes(get_option( $value['id'])  ); } else { echo $value['std']; } ?>" <?php if(!empty($value['size'])) : ?>size="<?php echo $value['size']; ?>" <?php endif; ?> />
  	</div>
  	<?php
  }
  ...
  function max_get_option_socialinput($value){
		...
		foreach($value['options'] as $index => $option) {
			...
			<li class="clearfix">
				...
				<div class="max_style_holder max_style_<?php echo $value['type'] ?>">
					<input class="socialurl max_control_input" name="<?php echo MAX_SHORTNAME ?>_social_<?php echo $index  ?>" id="invictus_social_<?php echo $index ?>" type="text" value="<?php echo stripslashes( get_option( MAX_SHORTNAME.'_social_'.$index  )  ); ?>" />
				</div>
			</li>
			<?php
			$i++;
