...
global $showSuperbgimage, $fromGallery, $taxonomy_name, $the_term, $post_terms, $isFullsizeGallery, $isPost, $isBlog, $shortname, $main_homepage, $_query_has_videos, $isFullsizeFlickr, $infiniteScroll, $max_retina_sizes;
...
?>
		<div id="superbgimage">
          ...
          if(!empty($_gallery_terms)) :
            ...
            $retinaUrl = max_get_post_image_url(get_the_ID(), $max_retina_sizes[max_get_image_string()]);

						echo '<a class="item" href="'. $imgUrl .'" data-1x="'. $imgUrl .'" data-2x="'. $retinaUrl[0] .'"></a>';
				...
				if( $_background_type == 'slideshow' && $random_post && !is_category() && !is_tax() && !is_archive() && !is_404() && !is_tag() && !is_search() ){
  				...
  				$random_post = max_query_term_posts( -1, $_gallery_array, 'gallery', 'rand' );
					...
					if (have_posts()) : while (have_posts()) : the_post();
						...
						// get the retina version
						$retinaUrl_temp = max_get_post_image_url(get_the_ID(), $max_retina_sizes[max_get_image_string()]);
						$retinaUrl = $retinaUrl_temp[0];

            echo '<a class="item" href="'. $imgUrl .'" data-1x="'. $imgUrl .'" data-2x="'. $retinaUrl .'"></a>';
					...
					if( $imageURL == "" ){
						...
						// get the retina version
						$retinaUrl_temp = max_get_post_image_url($random_post[0]->ID, $max_retina_sizes[max_get_image_string()]);
						$retinaUrl = $retinaUrl_temp[0];
			...
			if( $imgUrl != "" && $_background_type != 'slideshow' ){

        $retinaUrl = !$retinaUrl || $retinaUrl == "" ? $imgUrl : $retinaUrl;

				$img_output = '<a class="item" href="'. $imgUrl .'" data-1x="'. $imgUrl .'" data-2x="'. $retinaUrl .'" ></a>';
