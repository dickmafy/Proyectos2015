...
<div id="single-page" class="clearfix left-sidebar">
		...
		<div id="primary">
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
                ...
                $imgUrl = max_get_custom_image_url(get_post_thumbnail_ID(), get_the_ID(), 660, $timb_height, get_cropping_direction( $meta[MAX_SHORTNAME.'_photo_cropping_direction_value'] ) );

							}else{
                // get the default image if we don't have to crop
                $imgUrl = max_get_image_path(get_the_ID(), 'content');
              }
            ...
            $gallery_posts = new WP_Query($gallery_args);
                  ...
                  // get the default image if we don't have to crop
                  $_wp_image = max_get_post_image_url(get_the_ID(), 'mobile');
                  $_imgUrl = theme_thumb($_wp_image[0], 400, 300, get_cropping_direction( get_post_meta(get_the_ID(), MAX_SHORTNAME.'_photo_cropping_direction_value', true) ), false);
		...
		<div id="sidebar">
		  <?php
		  wp_reset_query();
      /* Widgetised Area */	if ( !function_exists( 'dynamic_sidebar' ) || !dynamic_sidebar( max_get_custom_sidebar('sidebar-gallery-project')) );
      ?>
		</div>
		<?php endif; ?>
