...
<table width="100%" height="91" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="1195" align="right" valign="middle" class="negrita14">Actualizar Empleador</td>
  ...
  <tr>
    ...
    <table width="554" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <span class="texto">En esta sección podrás actualizar los datos de un empleador registrado en el 
        sistema</span>
        </td>
        </tr>
    </table>
  ...
  <tr>
    ...
    <form id="form" name="form" method="post" action="proc_actualizar_empleador.php" enctype="multipart/form-data">
      <div class="container">
      <ul class="tabs">
	  ...
	  if($_POST["idempleador"]!=""){$idempleador = $_POST["idempleador"];}else{$idempleador = $arr_datos[0];}
	  ...
	  if($_POST["fecha_nac"]!=""){$fecha_nac = $_POST["fecha_nac"];}else{$fecha_nac = $arr_datos[8];}
	  if($_POST["hijos"]!=""){$hijos = $_POST["hijos"];}else{$hijos = $arr_datos[9];}
	  if($_POST["nro_hijos"]!=""){$nro_hijos = $_POST["nro_hijos"];}else{$nro_hijos = $arr_datos[10];}
	  if($_POST["mascota"]!=""){$mascota = $_POST["mascota"];}else{$mascota = $arr_datos[11];}
	  if($_POST["pais"]!=""){$idpais = $_POST["pais"];}else{$idpais = $arr_datos[12];}
	  if($_POST["tipo"]!=""){$tipo = $_POST["tipo"];}else{$tipo = $arr_datos[13];} 
	  if($_POST["nombre"]!=""){$nombre = $_POST["nombre"];}else{$nombre = $arr_datos[14];}			
	  if($_POST["dpto"]!=""){$dpto = $_POST["dpto"];}else{$dpto = $arr_datos[15];} 
	  if($_POST["manzana"]!=""){$manzana = $_POST["manzana"];}else{$manzana = $arr_datos[16];} 
	  if($_POST["lote"]!=""){$lote = $_POST["lote"];}else{$lote = $arr_datos[17];}
	  if($_POST["urbanizacion"]!=""){$urbanizacion = $_POST["urbanizacion"];}else{$urbanizacion = $arr_datos[18];} 
	  if($_POST["referencia"]!=""){$referencia = $_POST["referencia"];}else{$referencia = $arr_datos[19];} 
	  if($_POST["departamento"]!=""){$iddepartamento = $_POST["departamento"];}else{$iddepartamento = $arr_datos[20];}					
	  if($_POST["provincia"]!=""){$idprovincia = $_POST["provincia"];}else{$idprovincia = $arr_datos[21];} 
	  if($_POST["distrito"]!=""){$iddistrito = $_POST["distrito"];}else{$iddistrito = $arr_datos[22];}  
	  if($_POST["telefono"]!=""){$telefono = $_POST["telefono"];}else{$telefono =  $arr_datos[23];}
	  if($_POST["tipo_cel"]!=""){$tipo_cel = $_POST["tipo_cel"];}else{$tipo_cel =  $arr_datos[24];}
	  if($_POST["celular"]!=""){$celular = $_POST["celular"];}else{$celular = $arr_datos[25];} 					
	  if($_POST["adultos"]!=""){$adultos = $_POST["adultos"];}else{$adultos = $arr_datos[26];} 
	  if($_POST["desc_adultos"]!=""){$desc_adultos = $_POST["desc_adultos"];}else{$desc_adultos = $arr_datos[27];}
	  if($_POST["ninos"]!=""){$ninos = $_POST["ninos"];}else{$ninos = $arr_datos[28];} 
	  if($_POST["desc_ninos"]!=""){$desc_ninos = $_POST["desc_ninos"];}else{$desc_ninos = $arr_datos[29];} 
	  if($_POST["mascotas"]!=""){$mascotas = $_POST["mascotas"];}else{$mascotas = $arr_datos[30];} 
	  if($_POST["desc_mascotas"]!=""){$desc_mascotas = $_POST["desc_mascotas"];}else{$desc_mascotas = $arr_datos[31];} 
	  if($_POST["email"]!=""){$email = $_POST["email"];}else{$email = $arr_datos[32];} 
	  if($_POST["se_entero"]!=""){$se_entero = $_POST["se_entero"];}else{$se_entero = $arr_datos[33];} 
	  ?>     
      <div class="tab_container">
      <div id="tab1" class="tab_content">
      <input name="idempleador" type="hidden" id="idempleador" value="<?php echo($idempleador);?>" />
