...
<script src='<?php echo get_template_directory_uri(); ?>/js/jwplayer/jwplayer.js?ver=3.2.1'></script>
		...
		<div id="responsiveTitle">
		  <a href="#"><span class="imagetitle">Show Details</span></a>
		</div>
		<?php } ?>
					...
					<div id="fullsize" class="clearfix pulldown-items">
						...
						if (have_posts()) : while (have_posts()) : the_post();
							...
							<?php if ( has_post_thumbnail() || $isFullsizeVideo) { ?>
								...
								$data_add .= "\"excerpt\":\"". htmlentities(get_the_excerpt(), ENT_QUOTES, "UTF-8") ."\",";
								...
								<a <?php echo $data_add ?> class="item <?php echo $_item_type .' '; echo $img_greyscale ?>" href="<?php echo $imgUrl_big; ?>" title="<?php the_title() ?>">
									...
									<img src="<?php echo $imgUrl; ?>" height="<?php get_option_max('fullsize_thumb_height',true) ?>" <?php echo $_img_width ?> class="is-thumb img-color" title="<?php strip_tags(the_title()) ?>" alt="<?php if(get_the_excerpt()){ echo strip_tags(get_the_excerpt()); }else{ echo strip_tags(get_the_title()); } ?>" />

									<?php
									if( get_option_max('fullsize_use_greyscale') == 'true' ) {
										// get the imgUrl for showing the post image
										$imgUrl_grey = max_get_custom_image_url(get_post_thumbnail_ID(get_the_ID()), get_the_ID(), $_tim_width, get_option_max('fullsize_thumb_height'), $_cropping, true );
										?>
										<img src="<?php echo $imgUrl_grey; ?>" height="<?php get_option_max('fullsize_thumb_height',true) ?>" <?php echo $_img_width ?> class="is-thumb img-grey" title="<?php strip_tags(the_title()); ?>" alt="<?php if(get_the_excerpt()){ echo strip_tags(get_the_excerpt()); }else{ echo strip_tags(get_the_title()); } ?>" />
									<?php
									}
									?>
