...
if ( !post_password_required() ){ ?>
	...
	<?php if ($portfolio_posts->have_posts()) : ?>
		...
		<ul id="portfolioList" class="clearfix portfolio-list loading">
  		...
  		if( get_option_max('image_always_lightbox') == 'true' && $lightbox_class != 'link' ) :
    		$lightbox_class .=' detail-link';
  		endif;
  		?>

			<li data-time="<?php the_time('m/d/Y H:i') ?>" data-modified="<?php the_modified_time('Y-m-d H:i') ?>" data-id="id-<?php echo get_the_ID() ?>" class="item <?php echo $lightbox_class . " "; ?><?php echo $_term_classes . " "; ?><?php if( get_option_max( 'image_show_caption' ) == "always") { echo("show-title"); } ?><?php if( get_option_max('image_show_fade') != "true") { echo("no-hover"); } ?>">
				<div class="shadow">
     		...
     		if(get_option_max('image_always_lightbox') == 'true' && $lightbox_class != 'link'  ) :
     		?>
  		  <a href="" class="hover-link-icon"><?php _e('Link to detail page', MAX_SHORTNAME) ?></a>
  		  <?php endif; ?>
